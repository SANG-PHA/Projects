...
public class PushupActivity extends AppCompatActivity {
    ...
    private int pushupBaseGyroZPeakIdx_each = 84;
    private double pushupBaseGyroZPeakValue_each =  4.0;
    private int pushupBaseGyroZStartIdx_each =59;
    private int pushupBaseGyroZEndIdx_each = 79;
    private int pushupBaseGyroZ_Distance = pushupBaseGyroZEndIdx_each - pushupBaseGyroZStartIdx_each; // 20



    private int[] pushupBaseGyroZPeakIdx = {84,124,162,200,234,269,303,337,367}; // 원본과 -62 인덱스차이
    private double[] pushupBaseGyroZPeakValues = {1.408566,1.408036,1.355374,1.270685,1.39697,1.101579,1.157456,1.352634,1.149661};

    // ----------------- 여기까지 Baseline Data -----------------

    // 윗몸일으키기 User's data
    double[] pushupUserGyroZ = new double[10000];
    private ArrayList<Integer> pushupUserGyroZPeakIdx = new ArrayList<>();
    int PeakIdx = 0;
    double PeakValue = 0;
    int EndIdx = 0;
    double pre_val = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < 2 * pushupBaseGyroZ.length; i++)
        {
            float val = (float) pushupBaseGyroZPeakValue_each;
        ...
        mGyroLis = new PushupActivity.GyroscopeListener();

        mAccelometerSensor = mSensorManager2.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        mAccLis = new PushupActivity.AccelometerListener();
    ...
    private void compare_each(double value, int idx)
    {
        ...
        if(EndAct) {
            start++;
            // 정확도 계산
            if(PeakValue > pushupBaseGyroZPeakValue_each)
            {
                acc += (pushupBaseGyroZPeakValue_each - (PeakValue - pushupBaseGyroZPeakValue_each)) / pushupBaseGyroZPeakValue_each;
            }
            else
            {
                acc += PeakValue / pushupBaseGyroZPeakValue_each;
            }

            // ------------- Compare -------------
            double difference = 0.05; // 오차
            // 1. 동작 크기 비교
            if (PeakValue > pushupBaseGyroZPeakValue_each * (1 + difference)) // 동작이 크다
            {
                big++;

            } else if (PeakValue < pushupBaseGyroZPeakValue_each * (1 - difference)) // 동작이 작다
            {
                small++;
            ...
            // 2. 동작 속도 비교
            if (PeakIdx - EndIdx > pushupBaseGyroZ_Distance + 4) // 동작이 느리다
            {
                slow++;

            } else if (PeakIdx - EndIdx < pushupBaseGyroZ_Distance - 4) // 동작이 빠르다
            {
                fast++;
