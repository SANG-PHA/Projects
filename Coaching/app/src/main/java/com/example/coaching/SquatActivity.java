...
public class SquatActivity extends AppCompatActivity {
    ...
    private int squatBaseGyroZPeakIdx_each = 85;
    private double squatBaseGyroZPeakValue_each = -3.5;
    private int squatBaseGyroZStartIdx_each = 85;
    private int squatBaseGyroZEndIdx_each = 92;
    private int squatBaseGyroZ_Distance = squatBaseGyroZEndIdx_each - squatBaseGyroZStartIdx_each; // 7


    private int[] squatBaseGyroZPeakIdx = {84, 124, 162, 200, 234, 269, 303, 337, 367, 400}; // 원본과 -62 인덱스차이
    private double[] squatBaseGyroZPeakValues = {1.408566, 1.408036, 1.355374, 1.270685, 1.39697, 1.101579, 1.157456, 1.352634, 1.149661, 1.21321};

    // ----------------- 여기까지 Baseline Data -----------------

    // 윗몸일으키기 User's data
    double[] squatUserGyroZ = new double[10000];
    private ArrayList<Integer> squatUserGyroZPeakIdx = new ArrayList<>();
    int PeakIdx = 0;
    double PeakValue = 0;
    int EndIdx = 0;
    double pre_val = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < 2 * squatBaseGyroZ.length; i++)
        {
            float val = (float) squatBaseGyroZPeakValue_each;
        ...
        mGyroLis = new SquatActivity.GyroscopeListener();

        mAccelometerSensor = mSensorManager2.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        mAccLis = new SquatActivity.AccelometerListener();
    ...
    private void compare_each(double value, int idx)
    {
        ...
        if(EndAct) {
            start++;
            // 정확도 계산
            if(PeakValue < squatBaseGyroZPeakValue_each)
            {
                acc += (Math.abs(squatBaseGyroZPeakValue_each) - (Math.abs(PeakValue) - Math.abs(squatBaseGyroZPeakValue_each))) / Math.abs(squatBaseGyroZPeakValue_each);
            }
            else
            {
                acc += Math.abs(PeakValue) / Math.abs(squatBaseGyroZPeakValue_each);
            }

            // ------------- Compare -------------
            double difference = 0.05; // 오차
            // 1. 동작 크기 비교
            if (PeakValue < squatBaseGyroZPeakValue_each * (1 + difference)) // 동작이 크다
            {
                big++;

            } else if (PeakValue > squatBaseGyroZPeakValue_each * (1 - difference)) // 동작이 작다
            {
                small++;
            ...
            // 2. 동작 속도 비교
            if (EndIdx - PeakIdx > squatBaseGyroZ_Distance + 1) // 동작이 느리다
            {
                slow++;

            } else if (EndIdx - PeakIdx < squatBaseGyroZ_Distance - 1) // 동작이 빠르다
            {
                fast++;
